...
{
  [ "$1" = "startup" ] && MESSAGETEXT="Minecraft container online (${CLUSTER}/${SERVICE})"
  [ "$1" = "shutdown" ] && MESSAGETEXT="Shutting down Minecraft Server (${CLUSTER}/${SERVICE})"

  ## SNS Option
  [ -n "$SNSTOPIC" ] && \
  echo "SNS topic set, sending $1 message" && \
  aws sns publish --topic-arn "$SNSTOPIC" --message "$MESSAGETEXT" | jq -c .

  ## EventBridge Option
  echo "Emitting $1 event"
  cat << EOF >> entries.json
  [
    {
      "Source": "minecraft.watchdog",
      "DetailType": "$1",
      "Detail": "{\"application\": \"${APPLICATION_NAME}\",\"environment\": \"${ENVIRONMENT_NAME}\",\"cluster\": \"${CLUSTER}\",\"service\": \"${SERVICE}\",\"message\": \"${MESSAGETEXT}\"}"
    }
  ]
EOF
  aws events put-events --entries file://entries.json | jq -c .
}
...
function zero_service ()
{
  ...
  aws ecs update-service --cluster $CLUSTER --service $SERVICE --desired-count 0  | jq -c .
  exit 0
}
  ...
  "Changes": [
    {
      "Action": "UPSERT",
      "ResourceRecordSet": {
        "Name": "_minecraft._tcp.$SERVERNAME",
        "Type": "SRV",
        "TTL": 30,
        "ResourceRecords": [
          {
            "Value": "0 0 25565 game.$SERVERNAME"
          }
        ]
      }
    },
...
EOF
aws route53 change-resource-record-sets --hosted-zone-id $DNSZONE --change-batch file://minecraft-dns.json | jq -c .
